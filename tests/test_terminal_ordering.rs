...
fn fifty_move_checkmate_is_loss() {
    let (outcome, _pv, _nodes) = solve_with_pv("7K/8/8/8/8/8/1Q6/k7 b - - 100 1");
...
fn fifty_move_stalemate_is_draw() {
    let (outcome, _pv, _nodes) = solve_with_pv("7k/8/8/8/8/8/2q5/K7 w - - 100 1");
...
fn touching_commoners_with_two_pieces_is_draw() {
    let (outcome, _pv, _nodes) = solve_with_pv("8/8/8/8/8/8/1K6/k7 b - - 0 1");
    assert_eq!(outcome, Outcome::Draw);
}
