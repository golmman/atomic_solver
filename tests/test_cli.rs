...
/// The removed live proof-tree flags must hit the unknown-option error path
/// and exit non-zero (plan7: the search CLI no longer builds proof trees).
#[test]
fn cli_removed_proof_tree_options_exit_nonzero() {
    for (flag, value) in [("--pt-size", "64"), ("--dump-path", "target/x.bin")] {
        let output = Command::new(cli_bin())
            .args([
                "--fen",
                MATE_FEN,
        ...
        assert!(
            ...
            "{flag} must be rejected, but the CLI exited successfully"
        );
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            stderr.contains("unknown option"),
            "expected the unknown-option error for {flag}:\n{stderr}"
        );
        // The search must not have run.
        assert!(
            !String::from_utf8_lossy(&output.stdout).contains("outcome:"),
            "a rejected invocation must not perform a search"
        );
    }
}
