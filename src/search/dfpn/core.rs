...
impl Search {
    ...
    pub(super) fn dfpn(
        ...
        if let Some(outcome) = pos.outcome_from_state(&state, &moves) {
            ...
            self.emit_proof_node(pos, outcome, 0, 0);
            return outcome;
        }
        ...
        if let Some(resolved) = self.try_use_tt(pos, tt_key, max_depth) {
            self.emit_proof_node(pos, resolved.outcome, resolved.depth, 0);
        ...
        if let Some(outcome) = outcome_to_store {
            self.emit_proof_node(pos, outcome, outcome_to_store_depth, work);
            outcome
        } else {
            Outcome::Draw
        }
