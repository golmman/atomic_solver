...
| df-pn vs PDS head-to-head | **Pawlewicz & Lew 2007, *Improving Depth-first PN-Search: 1+ε Trick*, §4** | The primary quantified comparison: Atari Go TT-size sweep (Table 1), 488 easy + 286 hard LOA solved-counts vs time limit (Tables 2–3), geometric-mean speed ratios (Table 4: enhanced df-pn 4.17× faster than enhanced PDS, 4.46× than PDS) | **In repo** (`docs/theory/epsilon-trick-2007/epsilon-trick-2007.pdf`, vendored pre-plan5) | Not selectable as the deep source — already mined (`dfpn/research_epsilon.md`, criterion 4); used as the evidence-table anchor instead. Its §4 PDS comparison was not covered by that extraction |
