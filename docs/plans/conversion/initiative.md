...
backlog #1, 2026-09-21); (e) stays open here. |
| 6 | **Threshold-cut-frame pricing** (handed over from `dfpn` at its dormancy, 2026-09-17) | `lean` plan9's diagnostic: 99.7–99.9% of AND-frame own child evals sit in **threshold-cut frames** (frames exhausting their DF-PN threshold without a refutation exit: m22 8,625,570/8,645,613; stress case 148,390,289/148,602,798). Cut frames never reach a refutation exit, so no move-ordering signal can touch them; reducing the mass means changing how unsolved-subtree exploration is **priced** (DF-PN threshold dynamics), the same territory as the plan10/11 bound-folding hazard and the EWS/MOPNS no-gos | **Diagnostic spike executed** (`plan6.md` Phase 0, `report6.md`, 2026-09-18): cut mass measured at 98.9% of stress FO child evals; the three pre-registered classes failed their probes (A: 19.8%/34.7% ceiling but needs in-run resumption state = reuse-widening; B: empty — `epsilon_ceil(second) − best = 1` in 93.5% of AND cuts, ε inert; C: refuted — churn broad-shallow, top-20 positions ≈ 0.04%). **GO via a fourth class the spike surfaced**: the partial-sum sweep short-circuit (stop a cut frame's initial sweep once the summed bound crosses its threshold) measures a 74.2% first-order ceiling (FO) / 73.0% (default), present on all controls, and passes the plan10 hazard test on paper (frame-local, no solved facts, no new cache). Mechanism plan opened as **`plan7.md`** | nodes | M | **closed — measured no-go** (`plan7.md` Phase 1, `report7.md`, 2026-09-18): the implemented sweep short-circuit (gated `CONV7_SPIKE=1`, with an all-solved-prefix soundness guard the sketch implied) measured a **net loss** — stress FO 3.28× more child evals with the 477-ply win lost (120 s timeout), default 2.35× with the win lost, m22 control 2.16× (wall 1.83×), quick suite 2/59 outcome changes (win→draw) and 28/59 cases > 2× evals; in-sweep savings (gross 1.29B on stress FO) are eaten by re-entry churn (96.3% of the run's evals at re-entries of early-cut positions) and by missed decisive children behind the crossing point. Bars 1/2/4 failed; spike reverted, tree byte-identical, fast gate green |
| 7 | **Default-ε decision: 0.125 → 0.375** (sized note handed over from `research` at its 2026-09-21 pivot, plan4 Phase 0 spin-off) | Raise `DEFAULT_EPSILON` (`src/search/dfpn/mod.rs`) from 0.125 to 0.375: the only measured global constant with no control regression (stress −8.7% = 227,834,433 child evals FO; m22 −12.7%; dec13 +2.6%; dec10 −14.5%; `research/measurements/plan4/plan4_derived.csv`), re-validated as the plan8 gate baseline (all controls reproduce exactly). ε=0.5 wins stress −37.3% but regresses controls; no schedule or conditioned-ε arm beats the constant (research plan4 Phase 1, plan8 arms) — the constant is the closed ε-surface's best candidate | stress −8.7% child evals, no control regression — **under the 10% GO bar** used elsewhere in this initiative; decision is adopt / close won't-fix (0.375 stays opt-in via `--epsilon`) / park | nodes | S (one-line const change + doc updates; validation = quick-suite outcome gate + stress/m22/dec13/dec10 reproduction against the recorded 0.375 baselines) | **closed — won't-fix** (`report8.md`, 2026-09-21): the Phase 1 validation matrix confirmed the default-mode surfaces (stress −13.5%, m22 −12.7%, dec10 −12.4%; dec13 +23.2%) and reproduced all eight recorded FO baselines exactly, but the suite gates failed — quick@0.375 57/59 (dec01, m23_white `win → draw` via 5 s timeouts) and thorough FO +33.4% total work with 34/66 cases regressing (nine >+50%) and two new timeout flips on previously-solving cases; no soundness divergence anywhere (`wrong=false` everywhere). The four-point "no control regression" sample does not generalize; 0.375 stays opt-in via `--epsilon`, `DEFAULT_EPSILON` unchanged |
...
claim of the retained contract, which now lives on as a design constraint for the parallel effort
(`parallel` initiative, opened 2026-09-21 from #4 here / `lean` #2).
AND-side ordering signals in
general stay in `lean` #5; #3 here is only the clock/shuffle-specific
signal. Wall-time micro-engineering stays in `lean`. The ε-surface decision
(default constant) is tracked here as backlog #7, handed over from the
`research` pivot (its plan4 Phase 0 spin-off note, `research/report4.md`/`report8.md`).
...
- **2026-09-18** — **plan7 Phase 1 executed; backlog #6 closed as a
  measured no-go** (`report7.md`). The partial-sum sweep short-circuit was
  implemented behind `CONV7_SPIKE=1` (temporary `spike7.rs` + hooks +
  `solve_stats` runner), with one soundness guard added beyond the plan's
  sketch: a crossing may only cut when the partial table contains an
  unsolved child — otherwise `is_solved_by_children` could derive an
  outcome from a subset (a solved-Draw child contributes `pn = INF` at AND
  frames); with the guard, every early-cut frame is provably one the full
  sweep also threshold-cuts. Gate-off identity anchors reproduced all five
  HEAD baselines exactly (stress FO PV byte-identical). Spike-on verdict:
  **net loss everywhere** — stress FO 817,976,676 evals vs 249,480,478
  (3.28×) with the win lost at the 120 s cap; default 794,858,304 vs
  338,094,183 (2.35×), win lost; m22/m23/m24 evals 2.16–2.71× (m22 wall
  1.83×); quick suite 2/59 outcome changes and 28/59 cases > 2× evals;
  repetition gate green. Anatomy: gross in-sweep savings 1.29B (overshoot
  ≈ 0, prefix-minimal stops) against 96.3% of the run's evals spent at
  re-entries of early-cut positions — weaker partial bounds re-price the
  threshold lattice into churn, and an early cut between the crossing
  point and a later decisive child forfeits win proofs the full sweep
  always finds. Bars 1/2/4 failed ⇒ **NO-GO** per the plan's pre-registered
  rule; all spike code reverted (tree byte-identical to HEAD; post-revert
  stress FO stdout md5 `ffd3d015e9aa1a99efc1a3dad6831ae8` matches the HEAD
  capture; `make test` green). The pricing lane (cut-frame mass) is
  exhausted at the diagnostic level; remaining levers: #4 parallel (with
  `lean` #2), #5 (c)/(e) reading, parked #2a. Raw artifacts:
  `docs/plans/conversion/measurements/plan7/`.

- **2026-09-21** — **backlog #7 opened; handover from `research`** (no
  code, docs only): the ε=0.375 default-ε candidate — measured in
  `research` plan4 Phase 0 (Pareto-improving on all four cases, under the
  10% GO bar) and used as the plan8 gate baseline — was referenced as
  "pending in `conversion`" by `research/report8.md` but had never been
  registered in this backlog. Registered here as sized note #7 with the
  decision options (adopt / close won't-fix / park); per repo convention
  the decision itself belongs to a plan, not a backlog row.

- **2026-09-21** — `plan8.md` drafted for backlog #7, scoped as a **decision plan, not a spike**: Phase 1 is a zero-code-change validation matrix on the two unmeasured surfaces from the `research` plan4 hand-off — default mode with PV refinement (stress/m22/dec13/dec10, fresh exact 0.125 re-captures first; the inherited default-mode stress baseline is 338,094,183) and the quick/thorough suite outcome matrix under `benchmark --json` — plus exact reproduction of the four recorded FO baselines at both ε as environment sanity. The decision branches are pre-registered: **adopt** (all five gates pass, incl. default-mode stress non-regression and m22 default wall ≤ 15 s for the golden-test margin), **close won't-fix** (a measured surface regresses while FO improves), or **park** (mixed default-mode results). Phase 2 (on adopt only) is the one-line `DEFAULT_EPSILON` flip, the m22 default-mode trajectory-golden re-baseline with mandatory `verify_ppv` validation of the new PV, `make test-full`, and doc updates. Per-case ε autotune (report4 question (d)) is explicitly a non-goal — the ε surface is closed (`research` H1/H2).

- **2026-09-21** — **plan8 Phase 1 executed; backlog #7 closed as won't-fix**
  (`report8.md`). Zero-code-change validation matrix on the current binary
  (`benchmark --filter` per case; the CLI does not expose `child_evals`, the
  JSON harness does). Environment sanity: all eight recorded FO baselines
  reproduce exactly at ε=0.125/0.375 (stress 249,480,478 / 227,834,433, m22
  14,156,269 / 12,351,299, dec13 3,822,602 / 3,921,011, dec10 4,262,128 /
  3,642,166), and the fresh default-mode stress capture reproduces the HEAD
  baseline 338,094,183 exactly. Default mode (unmeasured surface (a)) is
  confirmatory: stress −13.5% (292,293,215), m22 −12.7% (15,447,517, wall
  2.51 s — inside the 15 s golden margin), dec10 −12.4%, but dec13 +23.2%
  (4,822,604 → 5,942,001). The suite gates then failed: quick@0.375 is
  57/59 (dec01, m23_white flip win→draw via 5 s timeouts; no `wrong`
  anywhere) and thorough FO at 30 s is +33.4% total child evals with 34/66
  regressions (nine >+50%: m23_white +3653%, dec01 +3584%, dec40 +1265%,
  dec36 +626%, dec05 +491%, dec31 +207%, m24_white +120%, dec14 +77%,
  dec30 +71%) and two new timeout-status flips on previously-solving cases
  (dec01 burns 210M evals unfinished vs 5.7M at 0.125; m23_white 363M vs
  9.7M) — not boundary artifacts, and offset by genuine improvements
  (m21_black/m22_black now solve at 0.375). Repetition gate green incl. the
  cyclic rook at 0.375 via CLI. Verdict per the pre-registered rule:
  **CLOSE WON'T-FIX** — surfaces regress at 0.375 while FO improves on the
  gate object; the four-point "no control regression" sample was luck of
  the trajectory-chaos draw and does not generalize to the suite. Phase 2
  skipped; `DEFAULT_EPSILON` stays 0.125, `src/` byte-identical to HEAD,
  0.375 remains opt-in via `--epsilon`. Raw artifacts:
  `docs/plans/conversion/measurements/plan8/`.

- **2026-09-21** — **backlog #4 moved; handover to `parallel`** (no code,
  docs only): the parallel search design spike left this initiative for
  the newly opened `docs/plans/parallel/`, which also absorbed the parked
