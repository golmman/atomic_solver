//! TEMPORARY plan13 Phase 0 T0 confirmation runner — spike code, reverted
//! after measuring. Source archived under
//! `docs/plans/dfpn/measurements/plan13/` together with the raw logs.
//!
//! Runs one FEN either in default solve mode or as a bounded
//! `search_depth(depth)` run and prints the deterministic metrics (child
//! evals, nodes, exit reason, wall, PV length). Same measurement surface as
//! plan12's archived `plan12_run.rs`, minus the retired spike-summary hook.
...
fn main() {
    ...
    println!(
        "plan13_run result: fen=\"{fen}\" mode={} outcome={outcome:?} exit={} nodes={} child_evals={} fo_child_evals={} refine_rounds={} refine_evals={} wall={:.2}s pv_len={} pv_status={:?}",
        if depth.is_some() {
    ...
    println!("plan13_run pv: {}", pv_uci.join(" "));
}
