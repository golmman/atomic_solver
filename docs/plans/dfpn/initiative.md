...
**plan13** (backlog #5 closed 2026-09-15 by plan12's evidence-based no-go;
the horizon/threshold-pricing successor lever is **open as backlog #6**;
`plan13.md` drafted and reviewed; Phase 0 executed 2026-09-16 —
G-A GO via architecture R, G-B PASS, awaiting the user checkpoint before
Session B, see `report13_phase0.md`).
...
| 6 | Bounded pre-phase for small decisive regions (horizon/threshold pricing) | plan12's measured asymmetry: a region fixpoint decides ~420k positions with certificates in ~6 s while bounded DF-PN collapses **zero** frames in 300 s (91.3% unsolved frontier churn; lean9: 99.7–99.9% of AND-frame evals in threshold-cut frames). A detector-gated pre-phase (occupied ≤ 3 men, no pawns) decides the **root** via a non-DF-PN architecture — PN bounded prover, layered certificate fixpoint, or region-closure fixpoint (Phase 0 bake-off) — with mandatory replay-verified cycle-free certificates and no TT interaction (plan10/11 hazard excluded by construction) | ladder root >756M nodes/unproven → ≤ 20M child evals and ≤ 60 s, certified (plan12's criterion); byte-identical everywhere the detector does not fire | nodes; plus a `PvStatus` variant and a `preflight:` CLI line | M–L | **Phase 0 executed (2026-09-16, `report13_phase0.md`): G-A GO via R only, G-B PASS — architecture R (region-closure fixpoint) selected, awaiting user checkpoint.** Table-free bake-off on the KQvK ladder: R decides the root as a replay-verified, cycle-free WIN at 5,827,440 child evals / 0.67 s (exact DTM 15; 0.29× of the 20M-eval bar), all four positions dtm-matching (15/13/11/9), h=dtm−1 correctly UNKNOWN, dtm+2 free; P (PN prover) measured no-go (certifies dtm 9 at 81k evals / 0.03 s — 370× better than plan12 T1b — but tree-node memory explodes from dtm ~11; 8M and 16M node caps all censored, memory-bound not eval-bound); L (layered certificate fixpoint) measured no-go (5–25× per-ply layer growth, root eval-cap at layer 8, and 85/85 oracle WIN claims failed certificate replay — certificate-first facts overflow the board-set encoding). G-B: 240 sampled placements, 0 contradictions for all candidates; R also proved 120/120 sampled draws correctly (closure draw proofs are a free by-product). `src/` byte-identical; spike archived under `measurements/plan13/` |
