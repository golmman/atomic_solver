...
| [`parallel`](parallel/initiative.md)     | Parallel proof search (opt-in wall-time scaling)                     | **closed measured no-go (2026-09-21)**: plan1 reading round (Čížek 2025, JLPNS, Young 2016) excluded A/B/D by soundness/economics; plan2 sizing spike measured option C — the only constraint-satisfying shape — at 0.27× wall / 15.5× work inflation on m22 (`design_space.md`, `measurements/plan2/`); no parallel mode ships; deterministic parallelism stays a measured no-go (lean plan7) |
