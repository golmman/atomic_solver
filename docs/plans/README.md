...
| [`solve`](solve/initiative.md)           | Solve atomic chess from the starting position (verifiable proof artifact) | plan2 M1 gate fired **NO-GO** (censored-run solved sections are search-local, not substrate; `report2.md`). Rethink reduced to two untested assumptions; **plan3 drafted** (not yet executed): 2 h quiet-root finishability probe (128 MB / 1 GB TT arms) + the ±1-ply substrate measurement — pre-registered gates decide between close-with-artifact / sharpness-first rescope / step-by-step pilot |
