...
| [`conversion`](conversion/initiative.md) | Deep tempo/progression conversions (`make stress` class)             | #6 threshold-cut-frame pricing: plan6 diagnostic **GO** via the partial-sum sweep short-circuit (`report6.md`, 74.2% ceiling) → `plan7.md` open; #4 parallel spike with `lean` #2; #5 items (c)/(e) reading; #2a ordering guidance parked |
